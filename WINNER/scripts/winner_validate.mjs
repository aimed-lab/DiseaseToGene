...
const nodeSet = ranked.slice(0, 2000).map(r => String(r.gene_symbol).toUpperCase());
...
// ── compare ──
const pairs = [];
for (const [g, sv] of stored) { const i = idx.get(g); if (i !== undefined) pairs.push([sv, p[i] / maxP]); }
const n = pairs.length;
const rk = (vals) => { const o = vals.map((v, i) => [v, i]).sort((a, b) => a[0] - b[0]); const r = new Array(vals.length); o.forEach(([, i], k) => { r[i] = k + 1; }); return r; };
const ra = rk(pairs.map(x => x[0])), rb = rk(pairs.map(x => x[1]));
let sd = 0; for (let i = 0; i < n; i++) { const d = ra[i] - rb[i]; sd += d * d; }
const rho = 1 - (6 * sd) / (n * (n * n - 1));
let maxAbs = 0, sumAbs = 0;
for (const [a, b] of pairs) { const d = Math.abs(a - b); if (d > maxAbs) maxAbs = d; sumAbs += d; }

console.log(`\n=== Sparse rewrite vs the stored snapshot ===`);
console.log(`  genes compared     : ${n}`);
console.log(`  Spearman rho       : ${rho.toFixed(4)}`);
console.log(`  mean |difference|  : ${(sumAbs / n).toFixed(4)}`);
console.log(`  max  |difference|  : ${maxAbs.toFixed(4)}`);
console.log(rho > 0.99 ? `\n  PASS - the rewrite reproduces the app's ranking.\n`
                       : `\n  FAIL - investigate before trusting the full-interactome scores.\n`);
