...
const MODEL_PROFILES: Record<string, ModelProfile> = {
  ...
  'gpt-5.6-luna': { steps: 6, tools: true, compactReference: true, toolResultChars: 40_000 },  // room for a whole ranked drug table (rank_drugs, 40 rows)
...
- NEVER say "no evidence in our store" until find_in_store has looked. The specific tools each cover one question shape; a miss from one of them (a drug that is not a graph node, a gene outside the graph, a query_graph error) means only that THAT view was empty. Run find_in_store with the name in question; report what it finds with its wiki links; only when it returns genes_with_hits = 0 does the store have nothing, and say exactly that. Being outside the knowledge graph is NOT the same as having no evidence or no rank - a gene outside the graph still has evidence rows and a board rank.
...
- ANSWERING A DRUG OR COMBINATION QUESTION ("are there trials or publications proposing X and Y", "is X + Y supported"). Gather in this order, then lay the answer out in the shape below. Both halves are required; a correct set of facts in the wrong shape still fails the reader.
  ...
  • An EVIDENCE ASSESSMENT, four lines, each graded Not established / Preclinical / Supported / Established:
      Direct combination evidence: …
      Mechanistic rationale: …
      Individual drug clinical evidence: …
      Combination clinical evidence: …
    Grade only what your tools returned. Preclinical means animal or cell work; Supported means human clinical data; Established means it would change practice.
  • A CONCLUSION naming what KIND of claim this is, not just repeating the findings. e.g. "a preclinical combination hypothesis with direct experimental support, rather than a clinically validated combination." This is the sentence that stops a reader over-reading the evidence.
  • A short reading list, every entry carrying a PMID, DOI or NCT id.
  QUOTE THE NUMBERS when a returned abstract carries them. If a tool result contains a headline efficacy figure — response rate, median survival, hazard ratio — put it in, labelled: "median OS <value> vs <value> months (Europe PMC abstract, PMID <id>)". Grading clinical evidence "Supported" while withholding the figure that supports it is a weaker answer than it needs to be. That is a tool result and is allowed; a figure no tool returned is not.
...
- CONNECT BACK TO WHAT WE HOLD, whatever the question. This is not only for drug questions. Whenever a question touches genes, targets, pathways, mechanisms or drugs that bear on the loaded disease, bring it back to our data: which of the genes involved sit in this snapshot, where they stand on the board, how they relate in the network, what stored evidence says about them. That connection is the reason someone asks this app rather than a search engine, so make it rather than leaving the reader to. Two limits: only make the connection where it is genuinely relevant, and if nothing we hold bears on the question, say so in one line and move on instead of forcing it.
...
function setupRoutes() {
  ...
  const AGENT_TOOLS = [
    ...
    { name: 'find_in_store', description: 'Search EVERYTHING stored for this disease for a term - every evidence row (all axes, the raw value_json included) and every knowledge-graph node - and return each hit with the gene, the axis, the matching text, and its wiki page. Use it for any name the specific tools do not take: a drug (olaparib), a variant (G12C), a trial id, a pathway, a tissue, a paper, a mechanism word. Call it BEFORE ever saying "no evidence in our store" - a gene outside the graph still has evidence rows, and a drug that is no graph node is still named in clinical and druggability rows. A hit is a text match, not an answer: judge whether the row bears on the question.', parameters: { type: 'OBJECT', properties: { term: { type: 'STRING', description: 'What to look for. One name or phrase; case-insensitive.' }, axis: { type: 'STRING', description: 'Optional: restrict to one evidence axis (clinical, druggability, mutation, annotation, literature_epmc, ...).' }, limit: { type: 'NUMBER', description: 'max genes returned (default 40).' }, disease: { type: 'STRING' } }, required: ['term'] } },
    { name: 'query_graph', description: 'Walk the snapshot\'s stored KNOWLEDGE GRAPH - the only tool that starts from something other than a gene, and the ONLY way to rank drugs. From stored rows: which genes a DRUG targets here and its trials in this disease (drug_targets); which drugs hit several genes or a given set of genes (shared_drugs - "genes that work for the same drug"); RANK drugs by the evidence stored for this disease - trials here, phase, active status, approval, target standing (rank_drugs - use it for "rank / compare / strongest evidence" across drugs; NEVER rank a set of drugs with search_trials or search_literature); a GENE\'s drugs, trials, pathways, papers and partners (gene_links); the ranked members of a PATHWAY (pathway_members); what a TRIAL tests and what that drug hits (trial). Call this FIRST for any question naming a drug, a pathway or a trial, or asking which genes share something; then search_literature / search_trials only for what the graph does not hold. Every entity carries its wiki_url and every ranking states its rule.', parameters: { type: 'OBJECT', properties: { query: { type: 'STRING', description: 'drug_targets | shared_drugs | rank_drugs | gene_links | pathway_members | trial' }, name: { type: 'STRING', description: 'The drug, gene symbol, pathway name or NCT id the query is about (not used by shared_drugs).' }, genes: { type: 'STRING', description: 'shared_drugs / rank_drugs: comma-separated gene symbols - only drugs that target ALL of them.' }, drugs: { type: 'STRING', description: 'rank_drugs only: comma-separated drug names to rank. Omit to rank every drug that meets min_genes.' }, min_genes: { type: 'NUMBER', description: 'shared_drugs / rank_drugs: minimum number of targeted genes (default 3 / 1).' }, limit: { type: 'NUMBER', description: 'max items (default 25).' }, disease: { type: 'STRING' } }, required: ['query'] } },
  ];
...
Rules: fill every field only from what the pages actually say. Where the page does not report something, write exactly "none reported". Never answer from memory. source_url is required and must be a page you actually opened. page_kind matters: a conference abstract has had far lighter review than a journal article, so name it honestly rather than calling everything peer-reviewed. If you cannot retrieve the paper, reply {"error":"full text unavailable"}.`,
  ...
  async function execAgentTool(name: string, args: any, ctx: { disease?: string; snapshotId?: number; modality?: string; litWindow?: string; userText?: string }): Promise<any> {
    ...
    // ── find_in_store: a term, anywhere in what is stored for this disease ────────
    // The specific tools are hand-written views; every question shape they do not cover used
    // to read as "no evidence" even when the rows held it (olaparib: absent from the graph,
    // named in 15 stored clinical rows). This is the general lookup: a substring scan over
    // every cached evidence row's text and value_json, and every graph node. Milliseconds;
    // the snapshot is already in memory. Hits are text matches - the model judges relevance.
    if (name === 'find_in_store') {
      const term = String(args?.term || '').trim();
      if (term.length < 2) return { error: 'term must be at least 2 characters' };
      const axisFilter = String(args?.axis || '').trim();
      const limit = Math.max(1, Math.min(200, Number(args?.limit) || 40));
      const needle = term.toLowerCase();
      const entry = await loadSnapshotCached(Number(snap.id));
      const ranks = new Map<string, number | null>((entry.scores as any[]).map((r: any) => [String(r.gene_symbol).toUpperCase(), r.rank ?? null]));
      const geneUrl = (g: string) => wikiUrl.entity(String(snap.disease_name), Number(snap.id), 'gene', g);
      // Where in a JSON blob did the term occur? Return the field path(s) and a short excerpt,
      // so the model sees WHY the row matched (a drug in trials[].drug vs a word in a title).
      const findIn = (v: any, path: string, out: Array<{ field: string; excerpt: string }>, depth = 0) => {
        if (out.length >= 4 || depth > 6 || v == null) return;
        if (typeof v === 'string' || typeof v === 'number') { const s = String(v); const i = s.toLowerCase().indexOf(needle); if (i >= 0) out.push({ field: path, excerpt: s.slice(Math.max(0, i - 40), i + needle.length + 60).replace(/\s+/g, ' ') }); return; }
        if (Array.isArray(v)) { v.forEach((x, i) => findIn(x, `${path}[${i}]`, out, depth + 1)); return; }
        if (typeof v === 'object') for (const [k, x] of Object.entries(v)) findIn(x, path ? `${path}.${k}` : k, out, depth + 1);
      };
      const byGene = new Map<string, { gene: string; rank: number | null; wiki_url: string; hits: any[] }>();
      let rowsScanned = 0, rowHits = 0;
      for (const r of entry.evidence as any[]) {
        if (axisFilter && r.evidence_type !== axisFilter) continue;
        rowsScanned++;
        const raw = `${r.value_text ?? ''}\u0000${typeof r.value_json === 'string' ? r.value_json : JSON.stringify(r.value_json ?? '')}`;
        if (!raw.toLowerCase().includes(needle)) continue;
        rowHits++;
        const g = String(r.gene_symbol).toUpperCase();
        const where: Array<{ field: string; excerpt: string }> = [];
        if (String(r.value_text || '').toLowerCase().includes(needle)) where.push({ field: 'value_text', excerpt: String(r.value_text) });
        findIn(jparse(r.value_json), '', where);
        const b = byGene.get(g) ?? byGene.set(g, { gene: g, rank: ranks.get(g) ?? null, wiki_url: geneUrl(g), hits: [] }).get(g)!;
        if (b.hits.length < 6) b.hits.push({ axis: r.evidence_type, source: r.source, matched: where.slice(0, 3) });
      }
      const genes = [...byGene.values()].sort((a, b) => (a.rank ?? 1e9) - (b.rank ?? 1e9));
      // Graph nodes whose label or key carries the term (drugs, trials, pathways, papers, tissues).
      let nodes: any[] = [];
      try { const gi = await agentGraph(Number(snap.id)); for (const t of ['drug', 'trial', 'pathway', 'paper', 'tissue', 'variant']) for (const n of gi.byType(t)) if (String(n.label).toLowerCase().includes(needle) || String(n.key).toLowerCase().includes(needle)) { nodes.push({ type: t, label: n.label, degree: n.degree ?? null, wiki_url: wikiUrl.node(String(snap.disease_name), Number(snap.id), n.key) }); if (nodes.length >= 30) break; } } catch { /* graph is optional here */ }
      return {
        disease: snap.disease_name, snapshot_id: snap.id, term, axis: axisFilter || null,
        rows_scanned: rowsScanned, matching_rows: rowHits, genes_with_hits: genes.length, genes: genes.slice(0, limit), graph_nodes: nodes,
        how_to_read: `Text matches in the STORED evidence of ${snap.disease_name} (snapshot #${snap.id}): each gene lists the axes whose rows mention "${term}" and where (field + excerpt). A match is not a claim - a drug named in a gene's clinical row means a trial of that drug is recorded for that gene in this disease, not that the drug targets the gene; a word in a paper title is just a title. Cite as (stored evidence, snapshot #${snap.id}) with the gene's wiki_url. genes_with_hits = 0 means the term appears nowhere in this disease's store - THAT is the moment to say "no evidence in our store", and to offer search_literature / search_trials for the live sources.`,
      };
    }
    ...
    if (name === 'query_graph') {
      ...
      // A many-target drug is usually one protein FAMILY (TUBB/TUBA... for a taxane) or an
      // antibody-drug conjugate whose payload hits that family. Name it, so the model does
      // not present fifteen tubulins as fifteen independent mechanisms.
      const family = (genes: string[]): string | null => {
        if (genes.length < 3) return null;
        const pre = new Map<string, number>(); for (const g of genes) { const k = g.replace(/[0-9].*$/, '').slice(0, 4); pre.set(k, (pre.get(k) || 0) + 1); }
        const [k, n] = [...pre.entries()].sort((a, b) => b[1] - a[1])[0];
        return n >= 3 && n / genes.length >= 0.6 ? `${k}* x${n}${n < genes.length ? ` + ${genes.filter(g => !g.startsWith(k)).slice(0, 4).join(', ')}` : ''}` : null;
      };
      const ACTIVE = /RECRUITING|ACTIVE|ENROLLING|NOT_YET/i;
      // Everything the store holds about one drug in THIS disease, aggregated deterministically.
      const drugFacts = (key: string) => {
        const genes = gi.in(key).filter((e: any) => e.rel === 'targeted_by').map((e: any) => String(e.source).slice(5)) as string[];
        const trials = gi.out(key).filter((e: any) => e.rel === 'tested_in').map((e: any) => trialOut(e.target));
        const phases = trials.map(t => Number(t.phase)).filter(n => Number.isFinite(n));
        const rankedTargets = genes.map(g => ({ gene: g, rank: ranks.get(g) ?? null })).sort(byRank);
        const best = rankedTargets.find(t => t.rank != null)?.rank ?? null;
        const n = gi.node(key);
        return { ...drugOut(key), n_targets: genes.length, family: family(genes), best_target: rankedTargets[0]?.gene ?? null, best_target_rank: best,
          targets_preview: rankedTargets.slice(0, 6).map(t => t.gene), n_trials_here: trials.length, max_phase_here: phases.length ? Math.max(...phases) : null,
          active_trials_here: trials.filter(t => ACTIVE.test(String(t.status || ''))).length, trials: trials.slice(0, 8), _genes: genes, _approved: n?.props?.approved === true || /approved/i.test(String(n?.props?.stage || '')) };
      };
      // The ranking rule, stated so the answer can state it: what the store holds about the
      // drug IN THIS DISEASE first, then how far it got anywhere, then how strong its target is.
      const RANK_RULE = 'max_phase_here (trials in this disease, from the stored clinical rows) desc -> active_trials_here desc -> n_trials_here desc -> approved (any indication) -> best_target_rank asc -> n_targets desc. Stored facts only; no literature counts, no model judgement.';
      const rankScore = (d: ReturnType<typeof drugFacts>) => [d.max_phase_here ?? -1, d.active_trials_here, d.n_trials_here, d._approved ? 1 : 0, -(d.best_target_rank ?? 1e9), d.n_targets];
      const cmpRank = (a: ReturnType<typeof drugFacts>, b: ReturnType<typeof drugFacts>) => { const x = rankScore(a), y = rankScore(b); for (let i = 0; i < x.length; i++) if (x[i] !== y[i]) return y[i] - x[i]; return a.drug.localeCompare(b.drug); };
      const publicFacts = (d: ReturnType<typeof drugFacts>) => { const { _genes, _approved, ...rest } = d; return { ...rest, approved: _approved }; };
      ...
      if (q === 'shared_drugs' || q === 'rank_drugs') {
        ...
        const namedDrugs = String(args?.drugs || '').split(/\s*,\s*/).map(s => s.trim()).filter(Boolean);
        const minGenes = Math.max(1, Number(args?.min_genes) || (want.length ? want.length : q === 'rank_drugs' ? 1 : 3));
        let keys: string[] = namedDrugs.length ? namedDrugs.map(n => gi.findDrug(n)?.key).filter(Boolean) as string[] : gi.byType('drug').map((d: any) => d.key);
        const missing = namedDrugs.filter(n => !gi.findDrug(n));
        let rows = keys.map(drugFacts).filter(d => d.n_targets >= minGenes && want.every(g => d._genes.includes(g)));
        if (q === 'shared_drugs') rows.sort((a, b) => b.n_targets - a.n_targets || a.drug.localeCompare(b.drug)); else rows.sort(cmpRank);
        // Compact rows so the WHOLE set fits in one tool result: the previous shape (20 target
        // objects per drug) truncated at 14 of 40 and the model rightly refused to invent the rest.
        const out = rows.slice(0, Math.max(limit, 60)).map(d => { const pf = publicFacts(d); const { trials, ...compact } = pf; return q === 'rank_drugs' ? { ...compact, trials: trials.slice(0, 4).map(t => `${t.nct} p${t.phase ?? '?'} ${t.status ?? ''}`.trim()) } : compact; });
        return { disease: snap.disease_name, snapshot_id: snap.id,
          filter: [want.length ? `target all of ${want.join(', ')}` : null, namedDrugs.length ? `named: ${namedDrugs.join(', ')}` : null, `>= ${minGenes} snapshot gene target${minGenes === 1 ? '' : 's'}`].filter(Boolean).join('; '),
          found: rows.length, returned: out.length, ...(missing.length ? { not_in_graph: missing } : {}),
          ...(q === 'rank_drugs' ? { ranking_rule: RANK_RULE } : {}),
          drugs: out,
          how_to_read: HOW + (q === 'rank_drugs'
            ? ' The order IS the evidence ranking under ranking_rule - present it in that order, say the rule, and give each drug\'s phase / trials / approval as its reason. "Evidence" here means trials recorded for this disease plus approval and target standing; it is not efficacy, and a drug with no trials here has no disease evidence in the store (say so; search_trials can check the live registry for ONE drug if asked).'
            : ' Many-target drugs are usually a protein FAMILY (family names it, e.g. TUB* x15 = tubulins) or an antibody-drug conjugate whose payload hits that family - say so rather than presenting the family as independent targets. To order these by disease evidence, call rank_drugs.') };
      }
      if (q === 'gene_links') {
      ...
      return { error: `unknown query "${q}"`, available: KNOWN };
    }
  ...
  // Kept from the removed Jobs block: otFetch outlived it. diseaseSearchTerms() uses it to
  // pull Open Targets disease synonyms, which is what stopped literature counts being
  // keyed to a single ontology label.
  ...
  // An in-process queue that ran harvest/enrich/add-genes from the browser and wrote
  // straight into Oracle. Deleted rather than repaired: it could not work on the
  // deployed app for three independent reasons — Oracle is unreachable from outside the
  // university, the runner continued after the response on a host that freezes the
  // function at that point, and it persisted to data/jobs.json on an ephemeral disk.
  //
  // Nothing was lost. scripts/d2t.ts imports oracleService directly and never used these
  // endpoints, so harvesting from the command line is untouched and is how it was always
  // actually done. The three evidence-provider helpers that lived here were used by the
  // job runners and by nothing else, so they went with it.
