...
export const AXES: AxisDef[] = [
  ...
  { key: 'literature', tier: 5, cost: 'sec', label: 'Literature signal', question: 'Is interest established / rising?', type: 'soft', source: 'Europe PMC', color: '#2a78d6', evidenceType: 'literature_epmc', direction: 'pro', weight: 0.75, headline: true,
    measures: 'Publication velocity — share of this target\'s disease-relevant papers published in the last ~3 years (momentum).',
    provenance: 'Europe PMC full-text gene x disease query; recent_count / paper_count. ONE corpus scores the axis. PubMed is harvested and shown alongside as a precise per-paper annotation but is NEVER scored: it is rate-limited to a small arbitrary subset and returns ~20x fewer papers, so its velocity sits on a tiny denominator - scoring a mix of the two made genes incomparable (same gene, ~0.28 velocity gap).',
    caveat: 'Soft tier: by default it ranks rather than gates. Measures attention / momentum, not biological strength. Genes with fewer than 5 papers are left unscored (low-confidence) because recent/total is quantised noise on a tiny denominator.',
  ...
  { key: 'clinical', tier: 8, cost: 'sec', label: 'Clinical landscape', question: 'Is there trial activity / room?', type: 'soft', source: 'Open Targets', color: '#2a78d6', evidenceType: 'clinical', direction: 'pro', weight: 0.75, headline: true,
    measures: 'How many drugs hitting this target have a registered trial IN THIS DISEASE, and the highest phase those disease trials reached.',
    provenance: 'Open Targets target->drug->trial graph (drugAndClinicalCandidates.clinicalReports), scoped to the disease and its ontology descendants. Phase is read per-trial, never from the drug global maximumClinicalStage. Replaced a ClinicalTrials.gov free-text search that had no gene field and matched substrings (renin scored 234 for pancreatic). Genes with 0 are not stored.',
    caveat: 'Soft tier: ranks, never gates. A high score means the target is ALREADY being pursued clinically for this disease - it rewards crowded targets and says nothing about a novel one. Read 0 as "no clinical precedent yet" (neutral/novelty), not as a negative.',
    filter: { kind: 'range', field: 'trial_count', unit: 'drugs in trials', min: 0, max: 25, step: 1, default: 0, op: '>=',
      presets: [{ label: 'Any', value: 0 }, { label: 'Has a drug in trials', value: 1 }, { label: 'Active', value: 3 }, { label: 'Crowded', value: 10 }] } },
